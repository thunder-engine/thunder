...
VsBuilder::VsBuilder() {
    ...
    Process vswhere;
    if(vswhere.start("C:/Program Files (x86)/Microsoft Visual Studio/Installer/vswhere.exe", {"-latest", "-property", "installationPath"})) {
...
bool VsBuilder::buildProject() {
    if(m_outdated && !m_process.isRunning()) {
        ...
        TString path = mgr->cachePath() + "/" + platform + "/" + gMode + "/";
        if(mgr->targetPath().isEmpty()) {
            product += gEditorSuffix;
            m_artifact = path + gPrefix + product + gShared;
            args = {"mine-editor.vcxproj", TString("/p:Configuration=") + gMode, "/p:Platform=\"x64\"", "/verbosity:minimal", "/nologo"};
        } else {
            m_artifact = path + product + gApplication;
            args = {"mine.vcxproj", TString("/p:Configuration=") + gMode, "/p:Platform=\"x64\"", "/verbosity:minimal", "/nologo"};
        }
