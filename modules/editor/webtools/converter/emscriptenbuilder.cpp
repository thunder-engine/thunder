...
EmscriptenBuilder::EmscriptenBuilder() :
    ...
    m_libs = QStringList() << "engine" << "next" << "physfs" << "freetype" << "angelscript" << "bullet3" << "rendergl" << "bullet" << "angel" << "glfm" << "zlib";
}
...
bool EmscriptenBuilder::buildProject() {
    if(m_outdated && !m_progress) {
        ...
        m_artifact = mgr->cachePath() + "/webgl/" + mgr->projectName() + gApplication;
...
bool EmscriptenBuilder::isPackage(const QString &platform) const {
    return false;
}
