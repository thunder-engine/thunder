...
VkDescriptorSetLayout WrapperVk::createDescriptorSetLayout(const vector<VkDescriptorSetLayoutBinding> &layoutBinding) {
...
VkDescriptorPool WrapperVk::createDescriptorPool(const vector<VkDescriptorPoolSize> &poolSize, uint32_t count) {
...
VkPipelineLayout WrapperVk::createPipelineLayout(const vector<VkDescriptorSetLayout> &layouts) {
...
void WrapperVk::recreateDevice(VkSurfaceKHR surface) {
    ...
    for(const auto &queueFamily : queueFamilies) {
        ...
        if(queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT && presentSupport) {
            priorities = vector<float>(queueFamily.queueCount, 0.0f);
