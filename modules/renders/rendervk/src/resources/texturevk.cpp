...
void TextureVk::readPixels(int x, int y, int width, int height) {
    ...
    transitionImageLayout(original, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 1);
    ...
    transitionImageLayout(VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, original, 1);
...
void TextureVk::updateTexture() {
    ...
    if(!isRender()) {
        const Surface &src = surface(0);

        transitionImageLayout(m_initialLayout, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipCount());
        ...
        transitionImageLayout(VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, m_finalLayout, src.size());
    } else {
        ...
        transitionImageLayout(VK_IMAGE_LAYOUT_UNDEFINED, m_initialLayout, 1);
    }
...
void TextureVk::setImage(VkImage image, VkFormat format) {
    m_image = image;

    convertFormatFromNative(format);

    createImageView(WrapperVk::device(), format);
}
