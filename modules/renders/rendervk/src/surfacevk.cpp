...
static VkFormat s_depthStencilFormat = VK_FORMAT_UNDEFINED;
...
void SurfaceVk::recreateSwapChain() {
    ...
    for(auto it : swapChainImages) {
        ...
        m_images[index]->setImage(it, s_selectedFormat.format);
    ...
    m_depth->setImage(m_depth->createImage(device, s_depthStencilFormat), s_depthStencilFormat);
    ...
    attachDesc[1] = {};
    attachDesc[1].format = s_depthStencilFormat;
