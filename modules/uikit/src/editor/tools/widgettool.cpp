...
void WidgetTool::setTranslation(const Vector3 &position, const Vector3 &scale) {
    m_translationPosition = position;
...
void WidgetTool::update(bool pivot, bool local, bool snap) {
    ...
    center -= m_translationPosition;
    size.x /= m_translationScale.x;
    ...
    if(parent->layout()) {
        ...
        drawAnchors(center, size, minAnchor, maxAnchor);

        int axis;
        bool isDrag = m_controller->isDrag();
        m_world = Handles::rectTool(center, size, axis, false, isDrag);
        ...
        if(isDrag) {
            ...
            if(delta.length() >= 1.0f) {
                ...
                if(rect->parentTransform()) {
                    rect->setPosition(Vector3(min + size * rect->pivot(), m_position.z));
                }
...
void WidgetTool::drawAnchors(const Vector3 &center, const Vector3 &size, const Vector2 &minAnchor, const Vector2 &maxAnchor) {
    ...
    static const Vector3 cornerA(10.0f, -5.0f, 0.0f);
    static const Vector3 cornerB( 5.0f,-10.0f, 0.0f);
    static const Vector3 cornerC(10.0f,  5.0f, 0.0f);
    static const Vector3 cornerD( 5.0f, 10.0f, 0.0f);
