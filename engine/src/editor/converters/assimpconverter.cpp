...
Mesh *AssimpConverter::importMesh(const aiScene *scene, const aiNode *element, Actor *actor, AssimpImportSettings *fbxSettings) {
    if(element->mNumMeshes) {
        ...
        for(uint32_t index = 0; index < element->mNumMeshes; index++) {
            ...
            if(mesh == nullptr) {
                mesh = Engine::loadResource<Mesh>(info.uuid);
                if(mesh == nullptr) {
                    mesh = Engine::objectCreate<Mesh>(info.uuid);
                }
            }
        ...
        for(uint32_t m = 0; m < element->mNumMeshes; m++) {
            ...
            if(item->HasVertexColors(0)) {
                memcpy((void*)&colors[total_v], item->mColors[0], sizeof(Vector4) * vertexCount);
            }
            ...
            if(item->HasBones()) {
                ...
                memset((void*)&weights[total_v], 0, sizeof(Vector4) * vertexCount);
                memset((void*)&bones[total_v], 0, sizeof(Vector4) * vertexCount);
