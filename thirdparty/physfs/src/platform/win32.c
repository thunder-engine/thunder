...
static char *getExePath(const char *argv0)
{
    ...
    retval[0] = '\0';
    buflen = GetModuleFileNameA(NULL, retval, MAX_PATH + 1);
    if (buflen <= 0)
...
char *__PHYSFS_platformGetUserName(void)
{
    ...
    if (GetUserNameA(NULL, &bufsize) == 0)  /* This SHOULD fail. */
    {
        retval = (LPTSTR) malloc(bufsize);
        BAIL_IF_MACRO(retval == NULL, ERR_OUT_OF_MEMORY, NULL);
        if (GetUserNameA(retval, &bufsize) == 0)  /* ?! */
        {
            __PHYSFS_setError(win32strerror());
            free(retval);
            retval = NULL;
...
int __PHYSFS_platformExists(const char *fname)
{
    BAIL_IF_MACRO
    (
        GetFileAttributesA(fname) == PHYSFS_INVALID_FILE_ATTRIBUTES,
        win32strerror(), 0
    );
    return(1);
...
{
    return((GetFileAttributesA(fname) & FILE_ATTRIBUTE_DIRECTORY) != 0);
                                                  ...
                                                  int omitSymLinks)
{
    ...
    dir = FindFirstFileA(SearchPath, &ent);
    BAIL_IF_MACRO
    (
...
char *__PHYSFS_platformCurrentDir(void)
{
    ...
    buflen = GetCurrentDirectoryA(buflen, NULL);
    retval = (LPTSTR) malloc(sizeof (TCHAR) * (buflen + 2));
    BAIL_IF_MACRO(retval == NULL, ERR_OUT_OF_MEMORY, NULL);
    GetCurrentDirectoryA(buflen, retval);
...
static void *doOpen(const char *fname, DWORD mode, DWORD creation, int rdonly)
{
    ...
    fileHandle = CreateFileA(fname, mode, FILE_SHARE_READ, NULL,
...
int __PHYSFS_platformDelete(const char *path)
{
    ...
    {
        BAIL_IF_MACRO(!RemoveDirectoryA(path), win32strerror(), 0);
    } /* if */
    else
    {
        BAIL_IF_MACRO(!DeleteFileA(path), win32strerror(), 0);
    } /* else */
