...
void AbstractHighlighterPrivate::ensureDefinitionLoaded()
{
    ...
    if (Q_UNLIKELY(!m_definition.isValid() && defData->repo && !m_definition.name().isEmpty())) {
        qDebug() << "Definition became invalid, trying re-lookup.";
    ...
    if (Q_UNLIKELY(!defData->repo && !defData->fileName.isEmpty()))
        qCritical() << "Repository got deleted while a highlighter is still active!";
...
State AbstractHighlighter::highlightLine(const QString &text, const State &state)
{
    ...
    if (text.isEmpty()) {
        ...
        while (!stateData->topContext()->lineEmptyContext().isStay() || (stateData->topContext()->lineEmptyContext().isStay() && !stateData->topContext()->lineEndContext().isStay())) {
            ...
            if (endlessLoopingCounter > 1024) {
                qDebug() << "Endless switch context transitions for line empty context, aborting highlighting of line.";
                break;
            }
        }
    ...
    do {
        ...
        if (lastOffset == offset) {
            ++endlessLoopingCounter;
            if (endlessLoopingCounter > 1024) {
                qDebug() << "Endless state transitions, aborting highlighting of line.";
                break;
            }
        } else {
     ...
     */
    {
        ...
        while (!stateData->topContext()->lineEndContext().isStay() && !lineContinuation) {
            ...
            ++endlessLoopingCounter;
            if (endlessLoopingCounter > 1024) {
                qDebug() << "Endless switch context transitions for line end context, aborting highlighting of line.";
                break;
            }
        }
    }
